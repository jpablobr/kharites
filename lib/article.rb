...
module Kharites
  class Article
    ...
    def load_articles_directories
      Dir[File.join(Configuration.data_directory, '*')].select \
        { |dir| File.directory?(dir) }.sort
    end
    ...
    def extract_article_info_from(file)
      raise ArgumentError, "#{file} is not a readable file" unless
        File.exist?(file) and File.readable?(file)
